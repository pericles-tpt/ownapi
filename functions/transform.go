...
func RegenerateUserCodeAsSharedObjectGo(components FileComponents, inpPaths, outPaths []string) error {
	...
	err = generateFiles(components, inpPaths, outPaths)
	if err != nil {
		return err
	}
...
func generateFiles(components FileComponents, inpPaths []string, outPaths []string) error {
	sb := strings.Builder{}

	pre := fmt.Sprintf("/* THIS FILE WAS GENERATED BY ownapi AT %v FROM THE FOLLOWING INPUT PATHS:\n\t * %s\n*/\n", time.Now().Format(time.DateTime), strings.Join(inpPaths, "\n\t * "))
	sb.WriteString(pre)
