...
2. Make sure the package line is: `package main`
...
4. As long as your code is valid go, follows the above rules and only uses the supported imports, it'll be made available at runtime in `ownapi`
5. You can use the included `main.go` to test your functions, anything in that file will be ignored for code generation. To run `main()` use this command (from root, to run from another path just update the path accordingly):
```
go run ./user_functions/
```
